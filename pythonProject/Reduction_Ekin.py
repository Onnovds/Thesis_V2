...
# Initial guess
initial_guess_protons = [1, 1e4, 2]
initial_guess_e_all = [0.1, 1, 1.5]
initial_guess_e_primaries = [0.1,1, 1.5]
initial_guess_photons = [1, 1e4, 2]

# Fit
popt_protons, pcov_protons = curve_fit(inverse_power_law, E_gen, Reduction_factor, p0=initial_guess_protons)
...
# Plot
#protons
x_fit_protons = np.linspace(min(E_gen), max(E_gen), 500)
y_fit_protons = inverse_power_law(x_fit_protons, *popt_protons)
#electrons
...
plt.scatter(E_gen, Reduction_factor, color='red', label='Data points')
...
plt.title('Inverse Power Law Fit of Proton Energy Reduction Factor due to 5mm Aluminium Shielding')
plt.legend()
plt.grid(True)
...
plt.scatter(E_gen_e, Reduction_factor_e_all, color='blue', label='Electrons (all) data')
plt.plot(x_fit_protons, y_fit_e_all, color='blue', linestyle='--', label=f'Electrons (all) fit: a={popt_e_all[0]:.2f}, b={popt_e_all[1]:.1f}, c={popt_e_all[2]:.2f}')

plt.scatter(E_gen_e, Reduction_factor_e_primaries, color='green', label='Electrons (primaries) data')
plt.plot(x_fit_protons, y_fit_e_prim, color='green', linestyle='--', label=f'Electrons (primaries) fit: a={popt_e_prim[0]:.2f}, b={popt_e_prim[1]:.1f}, c={popt_e_prim[2]:.2f}')

plt.xlabel('Energy before shielding (MeV)')
plt.ylabel('Relative Energy Reduction Factor')
plt.title('Inverse Power Law Fit of Electron Energy Reduction Factor')
plt.legend()
plt.grid(True)
plt.show()
